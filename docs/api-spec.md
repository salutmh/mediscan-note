...
| `INVALID_RESET_CODE` | 400 | 재설정 코드가 없거나 만료·사용됨 (사유를 구분하지 않는다) |
...
### 1-4-2-1. POST /api/auth/password/reset — 비밀번호 재설정 (코드)

**로그인 없이 호출한다** — 비밀번호를 잊은 사람은 로그인할 수 없다.
운영자가 발급한 일회용 코드(2-A-3-2)로 새 비밀번호를 설정한다.

```json
{ "email": "user@example.com", "code": "ABCDE-FGHJK", "new_password": "..." }
```

**Response**
```json
{ "password_reset": true, "all_sessions_signed_out": true }
```

- 코드는 **일회용**이고 **24시간** 뒤 만료된다. 새 코드를 발급하면 이전 코드는 무효가 된다.
- 코드가 맞아도 **이메일이 다르면 거부**한다 (남의 코드를 들고 온 경우).
- 실패 사유를 구분해 알려주지 않는다 — "이 이메일이 가입돼 있는지"를 코드 대입으로
  알아낼 수 있으면 안 되기 때문이다. 전부 400 `INVALID_RESET_CODE` 에 같은 문구다.
- 성공하면 **모든 기기의 로그인이 끊긴다.** 계정을 되찾는 상황이므로 남의 세션이
  살아 있으면 안 된다.
- **토큰을 돌려주지 않는다.** 코드만 가진 사람이 곧장 세션을 얻는 것보다,
  새 비밀번호로 한 번 로그인하게 하는 편이 안전하다.
- 요청 수 제한: 1시간에 10회.
...
### 1-4-3. PATCH /api/auth/me/profile — 학습자 배경 (v0.8)

시안 02-2(프로필·학교) / 03(기본정보 설정)이 쓴다.

> **전부 선택 항목이다.** 직군·생년월일·학교·전공은 학습자 배경이고,
> 비어 있어도 모든 기능이 그대로 동작한다. 어떤 항목도 필수로 두지 않으며,
> 건너뛴 사람을 다시 이 화면으로 끌고 오지 않는다.
> **의료 자격을 확인하는 값이 아니다** — 직군으로 잠기거나 열리는 기능은 없다.

**Request** (보낸 키만 바뀐다. 빈 문자열은 **지우겠다**는 뜻이다)
```json
...
`GET /api/auth/me` 응답에도 같은 `profile` 객체가 함께 나간다.
탈퇴하면 `users` 행과 함께 사라진다.
...
>
> **"병변 없음" 답과 빈 기준 마스크 (v0.9)**
>
> 응답에 `answer_type`(`roi` | `no_abnormality`)과 `evaluation.reference_empty`(이 케이스의
> 전문가 기준 마스크에 표시된 병변이 없는지 — **채점 후에만 공개**)가 붙는다.
> 빈 쪽이 있으면 Dice 분모가 0 이 되므로 명시적으로 나눠 판정한다 (기준 마스크만 본다, AI 예측 무관):
>
> | 기준 마스크 | 답 | grade | dice/iou | location_score | 비고 |
> |---|---|---|---|---|---|
> | 빔 | `no_abnormality` | `match` | 1.0 | 100 | 놓친 0% / 과하게 0% |
> | 빔 | `roi` | `mismatch` | 0.0 | 0 | `user_precision: 0.0` (표시 전부가 기준 밖) |
> | 병변 있음 | `no_abnormality` | `mismatch` | 0.0 | 0 | `gt_coverage: 0.0`, `under_segmentation_ratio: 1.0` |
> | 병변 있음 | `roi` | 기존 Dice 판정 | | | 변경 없음 |
>
> 빈 기준 마스크는 케이스에 `reference_is_empty` 가 **명시적으로 켜져 있을 때만** 채점 기준으로 인정한다.
> 플래그 없이 비어 있거나, 플래그가 켜졌는데 병변이 있으면 등록 오류로 보고 422 `CASE_NOT_GRADABLE`.
> 문구는 "이 학습 케이스의 전문가 기준 마스크상 표시된 병변 없음"만 말하고 "정상"이라고 단정하지 않는다.

**ROI 입력 (`roi.type`)**

| 값 | 상태 | 설명 |
|---|---|---|
| `brush_mask` | 사용 | 브러시로 칠한 영역. `mask_png_base64` 필수 (빈 마스크는 400 — "병변 없음"으로 간주하지 않는다) |
| `no_abnormality` | 사용 (v0.9) | 학습자가 **명시적으로 고른 "병변 없음"** 답. 마스크를 보내지 않는다 (칠한 마스크를 함께 보내면 400) |
...
```
POST /api/cases/{case_id}/explanation-viewed     (로그인 필요)
요청 본문 없음
응답: { "recorded": true }
```

해설 블록이 **실제로 화면에 보였을 때** 프론트가 한 번 호출한다(IntersectionObserver).
채점 직후 해설은 항상 렌더되지만 사용자가 거기까지 내려봤는지는 다른 이야기라,
렌더 시점에 기록하면 "열람률 100%" 라는 쓸모없는 숫자가 나온다.

- 같은 (사용자, 케이스) 는 **한 번만** 쌓인다. `recorded: false` 는 실패가 아니라
  "이미 기록돼 있다" 는 뜻이다.
- 관찰용이라 실패해도 학습 흐름을 막지 않는다. 숨긴 케이스·없는 케이스는 404.
- 집계는 `GET /api/admin/learning-summary` 의 `explanations_viewed` /
  `submit_to_explanation_rate` 로 나온다. 분모는 **제출까지 간 (사용자, 케이스) 쌍**이다
  — 해설은 채점 뒤에 보이므로 열어보기만 한 사람을 분모에 넣으면 열람률이 실제보다 낮아진다.
...
### 2-6-1. GET /api/analyze/availability — 분석 가능 여부 (미리 확인)

화면이 **업로드 전에** 물어본다. 사용자가 영상을 올리고 ROI 를 칠하고 요청까지 한 뒤에야
"준비 중"을 만나면 그 노력이 통째로 낭비되기 때문이다.

```json
{
  "body_part": "brain_mri",
  "available": false,
  "unavailable_reason": "이 부위 모델은 volume(연속 슬라이스) 입력 전용이라 ...",
  "is_demo": false,
  "disclaimer": "본 결과는 학습 참고용 AI 분석이며 확정 진단이 아닙니다."
}
```

- 화면 문구를 **하드코딩하지 않는다.** 모델이 준비되면 문구가 반대로 거짓말을 하게 된다.
- 조회에 실패하면 화면은 **막지 않는다** — 확인을 못 했다고 기능을 잠그면 더 나쁘다.
  그 경우 요청 시점에 서버가 사실대로 답한다.
- `available: false` 면 요청 버튼이 비활성화되고 이유가 표시된다.
...
### 2-A-3-2. POST /api/admin/password-reset — 재설정 코드 발급
...
**Response**
```json
{ "code": "ABCDE-FGHJK", "expires_at": "2026-09-09T13:00:00+00:00", "user_id": "u_001" }
```

> ⚠️ **코드는 이 응답에만 존재한다.** 서버에는 SHA-256 해시만 저장되고 로그에도 남기지 않는다.
> 다시 볼 수 없으니 그 자리에서 사용자에게 전달해야 한다.

- 사용자 **목록 조회 경로는 만들지 않았다** — 운영자에게 전체 명단을 노출할 이유가 없다.
  이메일로만 지정한다.
- SNS 계정은 비밀번호가 없어 400 `PASSWORD_NOT_SET`.
- 발급 사실(누가 누구에게)은 애플리케이션 로그에 남는다. **코드 값은 남기지 않는다.**
