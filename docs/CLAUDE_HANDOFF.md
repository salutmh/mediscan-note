...
| 마지막 전체 검증 | 백엔드 **1066 passed** (SQLite·PostgreSQL 양쪽) / 프론트 **125 passed** / E2E 7종 / 접근성·좁은화면 점검 0건 / `verify_cases` 6케이스 |
...
**검증**: 백엔드 **1066 passed** / 프론트 **125 passed** / 빌드 통과 /
...
| 프론트 단위 | `cd frontend && npx vitest run` | **125 passed** |
| 빌드 | `npm run build` | 통과 |
