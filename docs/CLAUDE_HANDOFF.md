...
| 마지막 전체 검증 | 백엔드 **1008 passed** (SQLite·PostgreSQL 양쪽) / 프론트 **108 passed** / E2E 7종 / 접근성·좁은화면 점검 0건 / `verify_cases` 6케이스 |
...
사용자 평가: **"처음 Git에 올렸던 프로토타입과 눈에 보이는 차이가 거의 없다."**
맞는 지적이었다. 근본 원인은 디자인이 아니라 **데이터가 화면까지 오지 않는 것**이었다.

| 무엇이 문제였나 | 무엇을 했나 |
|---|---|
| `/` 가 케이스 격자로 바로 갔다 — "내가 어디까지 했는지"가 없음 | **홈 = 학습 대시보드** 신설 (`/api/me/dashboard`) |
| `/api/cases` 가 boolean 2개만 줌 — 카드에 시도 횟수도 못 씀 | 목록에 `progress` 추가 (시도·첫/최근/최고 일치도) |
| 복습노트가 "틀린 것 목록" | 최근·최고 점수 + 시도 횟수. 재도전 경과가 보인다 |
| 진행현황이 숫자 4개와 막대 1개 | 케이스별 학습 이력 표 + "다시 푼 것 중 몇 개가 나아졌나" |
| **영상이 화면 절반도 못 씀** (`max-width:560px` 하드코딩) | 부모가 폭을 정하게 하고 판독 화면은 화면 높이에 맞춤 |
| **되돌리기가 없음** (전체 지우기뿐) | 획 단위 undo/redo 25단계 + Ctrl+Z |
| 결과가 Dice·IoU·위치점수 세 숫자의 나열 | 판정 + 큰 숫자 하나 → 겹친/놓친/과한 영역 → 세부 지표는 접음 |

**찾아 고친 실제 버그**
- **방금 만든 기록이 "9시간 전"으로 보였다.** offset 없는 ISO 문자열을 내보내면
  브라우저가 로컬 시간으로 읽는다. `submitted_at` 은 `DateTime(timezone=True)` 지만
  **SQLite 는 시간대를 저장하지 않아 개발 환경에서만** 어긋났다 —
  배포 DB(PostgreSQL)로는 재현되지 않는 종류다. `app/timefmt.py` 로 모았다.
- **영상 로드 실패 문구가 서명 URL 을 그대로 화면에 뿌렸다** (서명·만료 파라미터 포함).
- **계약 검사기가 리스트의 첫 항목만 보고 있었다.** 케이스 목록의 첫 케이스가
  미시도면 `progress` 안의 필드가 통째로 검사에서 빠져, 문서에 없는 필드 5개가
  조용히 새어 나갔다.
- **좁은 화면에서 판독 캔버스가 6px 로 찌그러졌다** (내가 grid 로 바꾸며 만든 회귀).
  반응형 점검이 잡았다. 390 → 356px 로 복구.
- **검수 화면이 24장(9.3MB, 디코딩 67MB)을 페이지 로드 시 전부 받았다.**
  검수자는 첫 장을 보기까지 나머지 23장을 기다렸다. IntersectionObserver 로 바꿨더니
  그동안 타임아웃하던 case-review E2E 가 완주하기 시작했다.

**스테이징에서 학습 흐름 전체를 실제로 돌렸다 (`scripts/staging_e2e.py`, 33건 통과)**
가입 → 로그인 → 목록 → 상세 → slice → 제출 → **GT 기준 채점** → 해설 → 재도전 →
이력 → 대시보드 → 로그아웃 → **토큰 무효화** → 운영자 권한 거부 → 계정 정리.
이 PC 에는 AI sidecar 가 하나도 없어서, **AI 예측 없이 채점이 도는 것**을
실물로 확인했다 (`ai_prediction: null`, 채점 정상).

**검수 자동화에 빠져 있던 것 둘**
- 후보 간 중복 (같은 촬영이 다른 case_id 로 두 번). 24건 검사 결과 중복 없음.
  **자동으로 지우지 않는다** — 같은 환자의 다른 시점일 수 있어 사람에게 넘긴다.
- 출처 정보 완전성 6항목 (나중에 데이터셋 이용 조건을 확인할 근거)

**배포 점검에 추가**: `pg_dump` 존재 확인. 없으면 **차단**한다 —
첫 백업을 시도하는 순간(보통 배포 후)에야 알게 되는 구조였다.
이 PC 가 정확히 그 상태다 → 배포 전 `pg_dump` 있는 환경에서 `restore_drill` 필요.
...
**배포를 한 번도 안 해봐서 못 잡고 있던 것 두 개를 찾았다.**

| 무엇이 잘못돼 있었나 | 왜 안 보였나 |
|---|---|
| 연결 풀에 `pool_pre_ping` 이 없었다 | 로컬 Docker PostgreSQL 은 테스트 도중에 연결을 끊지 않는다. 원격 DB 는 끊는다 — 배포 후 첫 한산한 시간대에 500 이 난다 |
| 프론트 빌드에 **localhost 가 조용히 박혔다** | E2E 7종이 전부 Vite dev 서버라 한 번도 안 걸렸다. 그 dist 를 올리면 아무 요청도 나가지 않는다 |

`tools/staging/` — **배포와 같은 형태**를 로컬에 세운다 (production 빌드 + 정적 서버 +
다른 출처의 API + PostgreSQL). dev 서버로는 확인할 수 없는 것 15개를 검사한다:
빌드에 박힌 주소 · 실제 CORS preflight · `MEDISCAN_PUBLIC_BASE` 기준 자산 서명 ·
**SPA 깊은 경로 새로고침** · 오류 응답의 CORS.

`app/db_connection.py` — Supabase 연결 방식 판별. **포트만으로 구분하지 않는다**
(세션 풀러도 5432 를 쓴다). 마이그레이션·백업은 Direct 를 쓰라고 알린다.
`docs/SUPABASE_SETUP.md` 에 환경변수 목록과 절차가 있다.
...
**사람이 있어야 하는 것**
- **케이스 24건 육안 검수** — `/admin/review` 에서. 준비는 전부 끝났다.
  지금 6케이스로는 학습 분량이 부족하다 — **이게 제품을 가장 크게 바꾼다**
- **`case_findings` 입력** — 구조 완성, 전문가 대기 (BLOCKER-2)
- **Supabase 프로젝트 생성** — 연결 설계·검증 도구는 끝났다.
  Dashboard 의 Connect 에서 값을 받아 `docs/SUPABASE_SETUP.md` 절차를 따르면 된다
- **SNS 앱 등록** — 실검증 로직은 끝났다. 각 사 앱 ID 만 넣으면 켜진다

**코드로 가능한 것 (남은 것이 많지 않다)**
- 키보드로 ROI 입력할 수단 (설계 판단 필요)
- `Submission.explanation` 스냅샷을 어디에 보여줄지
- CSP 정책 확정 (배포 형태가 정해진 뒤 `MEDISCAN_CSP` 로)
- 본격 부하 테스트

**외부 수단·판단이 필요한 것 (BLOCKER)**
- 데이터셋·모델 이용 조건 (1) / 전문가 GT·소견 (2) / 규제 (3) /
  메일 발송 수단 (4) / 저장소 라이선스 (5)
...
| 무엇 | 명령 | 결과 |
|---|---|---|
| 백엔드 (SQLite) | `cd backend && pytest` | **1008 passed** |
