...
> **Phase 1~8 과 자율 루프 #1~17 이 전부 끝났다. 이미 있는 것을 다시 만들지 말 것.**
> `docs/RELEASE_READINESS.md` 3~6절에서 남은 항목을 보고, 아래에서 이어가면 된다.
>
> **1순위 — 사람이 있어야 진행되는 것 (지금 제품을 가장 크게 바꾼다)**
> - [ ] **케이스 확장**: 후보 24건이 선별돼 있다 (좌우 12:12 / 크기 8:8:8).
>       `scripts/screen_vs_seg_dataset.py` 재실행으로 목록을 다시 얻고,
>       export → **육안 검수** → 자산 → import → verify 순서로 등록한다.
>       육안 검수는 사람이 오버레이를 보는 자리라 건너뛸 수 없다 (BLOCKER-2 성격).
> - [ ] **`case_findings` 입력**: 구조는 완성돼 있다. 전문가가 `/admin/cases` 에서 입력하면 된다.
>
> **2순위 — 코드로 가능한 것**
> - [ ] 접근성 기본 점검 (키보드 이동, 대체 텍스트, 포커스 표시). 캔버스 드로잉은
>       본질적으로 한계가 있으니 그 외 화면부터.
> - [ ] 죽은 코드 정리: `_grade_by_points`(개발용 근사 채점), `cases.reference_shape`,
>       `frontend/public/icons.svg`(Vite 템플릿 잔재, 참조 없음).
>       reference_shape 제거는 마이그레이션이 필요하니 신중히.
> - [ ] 예측 sidecar 재계산 자동화 (지금은 수동 3단계)
> - [ ] `Submission.explanation` 스냅샷은 저장만 되고 아무도 읽지 않는다 — 유지할지 결정
>
> **3순위 — 외부 수단이 필요한 것**
> - [ ] 이메일 인증 (메일 발송 수단 필요 → BLOCKER-4)
> - [ ] 접속기록(감사 로그) 범위 (규제 판단 필요 → BLOCKER-3)
> - [ ] SNS 실인증 연동
>
> **작업 규칙 (이번 세션에서 얻은 것)**
...
> - 관리 화면에서 설정하는 값이 **학습자 경로까지 실제로 가는지** 확인한다 (반쪽 기능 방지)
> - UI 문자열에 API 경로·내부 필드명·문서 참조가 새지 않게 한다
> - 로깅처럼 **부작용이 눈에 안 보이는 코드**는 실행해서 출력을 확인한다
> - E2E 반복 실행은 `MEDISCAN_RATE_LIMIT=0` 으로 띄운 백엔드에서
