...
사용자 지적: *"ui가 너무 별로인데"* → *"전체 진행"* → *"사용자 입장에서 평가해줘"* → *"할 수 있는 건 전부 진행"*.

**(1) 화면이 평평해 보이던 원인은 색 토큰이 아니라 조판이었다** (`c714c3a`)
- 타입 스케일에 중간이 비어 있었다: h1 26 / h2 **17** / 본문 **15** — h2 와 본문이 2px 차이라
  섹션 제목이 본문처럼 읽혔다. `30 / 20 / 16 / 15` 로 다시 잡았다(모바일도 같은 비율).
- 표면이 한 종류였다: 홈 카드 **9개에 표면 1종**. `.card` / `.card.raised` / `.card.quiet` 3단으로 나눴다.
- `--shadow-sm` 이 5% 한 겹이라 배경(#f6f8fc) 위에서 사실상 안 보였다 — 겹을 하나 더 얹었다.
- 빈 상태가 채워진 칸만큼 자리를 먹었다(신규 홈의 40%). 데이터 없으면 카드가 `.quiet` 로 가라앉는다.
  **처음엔 빈 상태에 점선 박스를 줬다가 높이가 오히려 늘었다**(968 → 1039px). 박스를 걷고 카드를 가라앉혔다.

**(2) 토큰으로 못 고치는 구조 다섯 가지** (`8386435`)
- **결과 화면이 판독 화면 위에 결과를 덧붙인 형태였다.** 제출 뒤에도 잠긴 캔버스가 위 800px 을
  차지하고 옆 330px 칸은 90% 가 비어 있었다. 제출하면 캔버스를 접는다(펼 수 있게 남긴다).
  실측 **4,268 → 2,859px, "결과 비교" 1,732 → 323px**.
- 대시보드 아래 40% 가 백지 + 홈·진행현황과 같은 숫자 → **케이스별 최고 일치도 막대**를 넣었다.
  **시계열로 그리지 않았다** — 케이스마다 병변이 달라 선으로 이으면 없는 추세를 만든다.
  막대 색은 검증기로 골랐다(brand-500/600 은 채도 0.08 로 회색, brand-400 은 대비 2.81:1 → 둘 다 탈락, `#1f9aa3`).
- 오답 상세에서 영상이 1,300px 동안 빈 채 스크롤 → 좌측 sticky.
- 케이스 목록 부위 줄이 5칸 중 4칸 "준비 중" → **`showBodyPartCards` 가 주석과 다르게 동작**했다
  (`.some(ready)`). 실제로 고를 수 있을 때만 카드를 깐다.
- 카드의 "판독하기" 가 비활성처럼 보였다 → 글자 대비는 8.15:1 로 충분했고 문제는 **면**이었다.

**(3) 신규 계정으로 가입~채점을 직접 통과해 본 결과** (`fdb0723`)
- **가장 큰 것**: 기준 1,539px 병변에 **3,400px(2.2배)** 을 칠해 정상 조직으로 55% 가 넘쳤는데
  Dice 0.62 로 `match` → 학습완료 → **복습노트에서 빠졌다.** 같은 화면이 그 순간
  "경계를 조금 더 좁혀 보세요"라고 말하고 있었다 — **피드백과 등급이 다른 말을 하는데
  학습 경로는 등급만 따르고 있었다.** 이대로면 6케이스를 뭉툭하게 칠하고 "6/6 학습완료"가 된다.
  → `submissions.area_ratio` 저장 + `match` 라도 2배 초과면 복습에 담는다.
  **`grade` 는 건드리지 않았다.** 옛 제출(`area_ratio=None`)은 끌어오지 않는다.
- 첫 판독에서 무엇을 찾아야 하는지 배울 곳이 없었다(병변 생김새가 **제출 후 해설에만** 있었다).
  이미 커밋돼 있던 `imaging_features` 8개를 `/api/glossary` 로 내보내 사전 맨 위에 펼쳤다.
  **새로 지어낸 의학 내용은 없다.**
- 상단 메뉴 6개 중 4개가 겹쳤다 → `학습하기` 를 메뉴에서 뺐다(경로·화면은 유지, 홈에서 닿는다).
- `/learn` 에서 선택지가 1개인 단계를 세 번 누르게 했다 → 하나뿐이면 대신 고른다(둘 이상이면 그대로).
- 모바일에서 도구 레일이 영상을 덮었다 → 영상 아래 가로 줄로. **레일이 `overflow:hidden` 인
  `.stage-clip` 안에 있어** 처음엔 한 칸만 삐져나왔다 — 마크업을 `.stage-area` 로 옮겨 고쳤다.
- 로그인 첫 화면이 **안 되는 기능 3개**(SNS)로 시작했다 → 이메일을 위로, SNS 를 아래로.

**계약**: `api-spec.md` 에 `review` 블록 + `wrong-notes.review_reason` 추가(계약 테스트가 잡아 줬다).
**마이그레이션**: `e8ac296a80a0`(submissions.area_ratio, 추가 전용).

**검증**: 백엔드 1080 → **1092**(SQLite·**PostgreSQL 양쪽**, 마이그레이션 왕복 포함) /
프론트 194 → **214** / 빌드 / 접근성 0건 / 좁은화면 0건 /
E2E user-flow·error-paths·slice-navigation·roi-undo·consent-and-sns 통과.

> **확인하다 오해했던 것**: "가입 완료" 버튼이 이유 없이 잠긴 것처럼 보였는데, 사람 속도(150ms 간격)로
> 체크하면 정상이었다 — 자동화가 0ms 루프로 눌러서 생긴 것이라 **제품 결함이 아니다.**
> 보고 전에 확인했다. 또한 브라우저 확장의 좌표 프레임이 어긋나 클릭이 빗나갔고 그래서 JS 이벤트로
> 진행했다 — **손가락으로 실제로 칠하는 감촉은 검증하지 못했다**(특히 모바일).

> **못 한 것**: 6케이스가 전부 `뇌 MRI · 전정신경초종`이라 무엇부터 풀지 고를 근거가 없다.
> 구별 기준이 될 `difficulty` 는 전부 null 이고 **전문가 판정 항목(E2)** 이다. 병변 크기 같은 것을
> 대신 노출하면 제출 전에 정답 크기를 알려주는 셈이라 안 된다. 만들 수 있는 게 없어 비워 뒀다.
...
**02-2 — 가입을 시안처럼 두 단계로 나눴다.**
1단계 계정 정보(이메일·비밀번호) → 2단계 프로필·학교(이름·생년월일·학교·전공).
**동의는 시안에 없지만 반드시 받는다** — 2단계의 마지막, 즉 계정이 실제로 만들어지는
버튼 바로 앞에 뒀다. 계정 정보와 떼어 놓지 않으려던 앞선 판단을 여기서 바꿨는데,
**동의가 "가입 버튼 직전"에 있는 것이 오히려 맞다** (무엇에 동의하고 무엇이 만들어지는지가
같은 화면에 있다).
닉네임만 필수이고 생년월일·학교·전공은 **라벨에 "선택"이라고 적었다** —
필수처럼 보이면 가입을 망설인다. 선택 항목 저장이 실패해도 **가입은 유효하다**
(학교·전공 때문에 방금 만든 계정으로 못 들어가는 일은 없어야 한다).

**11 — 마이페이지에 설정 행과 지표 줄을 붙였다.**
프로필(생년월일·학교·전공) 수정, 직군 변경, 하단 지표 3칸(학습완료·기준과 일치한 비율·복습 필요).
**시안의 "학습 수준"은 넣지 않았다** — 우리에겐 수준을 쓰는 곳이 없어서
아무것도 하지 않는 설정이 된다 (죽은 컨트롤을 만들지 않는다).

**E2E 를 두 곳 고쳤다.** 가입이 2단계가 되면서 `user-flow` 와 `consent-and-sns` 가
1단계에서 멈췄다. 두 스크립트에 "다음" 단계를 넣고, 신규 가입 도착지를
`/cases` → `/onboarding`(건너뛰면 `/`)으로 기대값을 맞췄다.

**검증**: 백엔드 **1080** / 프론트 **140** / 빌드 / 접근성 0건 / 좁은화면 0건 /
E2E user-flow·consent-and-sns 통과.

---

### UI 개편 #7 — 시안 화면을 **독립 화면으로** 만들었다 (03 · 05 · 06)

사용자 지적: *"기본정보 설정도 메인화면도 학습대시보드도 학습선택도 다 다르잖아."*
맞다. 그 넷은 내가 **시안 프레임을 만들지 않고 우리 화면에 녹여 넣었다.**
특히 05(학습 대시보드)는 시안에서 04(메인)와 **다른 화면**인데 홈에 합쳐 뒀고,
그 위에 04 의 통계 띠로 KPI 카드를 덮어써서 결국 대시보드가 사라져 있었다.

**새로 만든 화면 3개**
| 시안 | 경로 | 내용 |
|---|---|---|
| 03 기본정보 설정 | `/onboarding` | 직군 4카드 + 시작하기. **가입 직후에만** 거친다 |
| 05 학습 대시보드 | `/dashboard` | KPI 3장(진도·기준과 일치한 비율·최고 일치도) + 기간 필터 + 최근 학습 표 |
| 06 학습 선택 | `/learn` | 좌측 사이드바 + 3단계(부위→영상→질환) + 학습 시작 |

홈(04)은 시안대로 **hero + 바로가기 3장 + 통계 띠**로 되돌렸다.

**03 은 건너뛸 수 있게 했다.** 시안에는 건너뛰기가 없지만, 직군은 학습자 배경일 뿐이고
비어 있어도 모든 기능이 그대로 동작한다. 여기서 막으면 **서비스를 보기도 전에
개인정보부터 내라는 화면**이 된다. 그리고 **의료 자격을 확인하는 화면이 아니다** —
고른 직군으로 잠기거나 열리는 기능을 만들지 않았다. 교육용 서비스에서 "의료인 인증"을
흉내 내는 게 더 위험하다.

**06 은 선택지를 실제 케이스 목록에서 만든다.** 화면에 하드코딩하면 케이스가 늘어도
여기만 옛 상태로 남는다. 준비되지 않은 부위·영상은 **누를 수 없고 "준비 중"이라고 적는다**
(예전에 부위 탭을 전부 깔았다가 빈 화면을 네 번 만나서 뺐던 그 문제를 되풀이하지 않는다).

**05 의 "정답률"은 `기준과 일치한 비율`로 쓴다.** 그리고 분모는 **전체가 아니라 시도한
케이스**다 — 아직 풀지 않은 것을 틀린 것으로 세면 시작도 안 한 사람이 0% 로 찍힌다.

**백엔드**: `users` 에 프로필 4열 추가(job_role/birth_date/school/major, 전부 선택),
마이그레이션 `cdf18c142374`(추가 전용), `PATCH /api/auth/me/profile`,
이메일 가입 응답에 `is_new_user` 추가(화면이 03 으로 보낼지 정한다).

**검증**: 백엔드 **1080** / 프론트 **140** / 빌드 / 접근성 0건 / 좁은화면 0건 /
E2E user-flow·a11y·responsive 통과.

---

### UI 개편 #6 — 시안/우리 것 중 나은 쪽을 골라 전부 반영

사용자가 **의견을 먼저 달라**고 해서 항목별로 평가했고, 그대로 진행했다.

**내가 잘못 판단했던 것을 먼저 적어 둔다.** 내 판단은 두 종류가 섞여 있었다 —
(a) 채점·문구·데이터 정직성은 우리가 낫고 (b) 화면의 모양·배치는 시안이 낫다.
그런데 (a)를 근거로 (b)를 정당화했다("우리 데이터엔 질환 선택이 없으니 레이아웃도 우리 것").
**모양에 관해서는 기본값을 시안으로 두는 게 맞다.**

#### 1. 오답 상세 (시안 10) — **피그마 채택.** 실제 구멍을 메웠다

해설은 **제출 직후에만** 볼 수 있었다. 복습노트에서 케이스를 누르면 바로 판독
화면으로 갔고, 해설을 다시 보려면 또 제출해야 했다 —
**틀린 것을 복습하러 와서 무엇을 틀렸는지 못 보고 다시 칠하는 구조였다.**

`GET /api/wrong-notes/{case_id}` 신설. **여기가 GT 노출의 경계다** —
기준 마스크는 `이 사용자가 이 케이스를 제출한 적이 있을 때만` 나가고,
없으면 404 `NO_SUBMISSION` 이다 (`tests/test_wrong_note_detail.py` 7개).

해설은 **제출 시점 스냅샷**(`submissions.explanation`)을 쓴다 —
"화면에 노출할 계획이 생기면 여기서 읽어 쓴다"고 남겨 둔 컬럼이 드디어 쓰였다.
사용자가 칠한 마스크는 저장하지 않으므로 **그 사실을 화면에 밝힌다**
(`user_mask_kept: false`) — 없는 것을 있는 것처럼 그리지 않는다.

시안의 "내 답안 vs 정답"은 **질환명을 고르는 퀴즈**(기관지염 vs 폐렴)라 우리 구조와
맞지 않는다. 같은 자리에 **내 일치도와 기준 영역**을 넣었다. 레이아웃은 시안 그대로다.

#### 2. 홈 통계 띠 (시안 04) — **피그마 채택**
시안 05 의 KPI 카드와 숫자가 겹쳐서 **띠 하나로 합쳤다.** 자세한 지표는 진행현황이 맡는다.
시안은 리드 칸 + 지표 3칸인데 우리는 보여줄 지표가 4개라 지표 칸만 하나 늘렸다
(총 시도가 사라진 것을 테스트가 잡았다).

#### 3. 부위 선택 카드 (시안 06) — **절반만 채택**
3단계 마법사는 버렸다 — 우리 데이터로는 **클릭 3번 해서 선택지 1개**에 닿는다.
대신 부위 카드를 목록 위에 얹었다. **준비되지 않은 부위는 누를 수 없고 "준비 중"이라고 적는다** —
예전에 부위 탭을 뺀 이유(빈 화면을 네 번 만난다)를 그대로 지키면서,
팀원 5명이 부위별로 모델을 맡는 제품의 폭은 보이게 했다.

#### 4. 화면 5 — **상단 메뉴에서 뺐다**
단일 이미지용 2D 모델이 없어 항상 "준비 중"만 뜬다. 그런 메뉴가 늘 떠 있으면
클로즈드 베타에서 손해다. 화면과 경로(`/analyze`)는 그대로 두고 **홈 바로가기**에서 닿게 했다.
**데모 모드는 켜지 않았다** — 가짜 분석 결과는 "데모"라고 적어도 위험하다.

#### 고치면서 찾은 것
- **옛 uvicorn 프로세스가 포트를 물고 있어** 새 라우트가 계속 404 였다.
  부모는 죽었는데 `multiprocessing` 자식이 소켓을 들고 있었다 (인수인계가 경고한 그 상황).
- **E2E 가 특정 구간에서 멈췄다.** 헤드리스에 page 타깃이 2개라 한쪽이 백그라운드로
  밀리면서 `setTimeout` 이 스로틀됐다 — 탭을 하나만 두면 통과한다.
- 새 에러 코드 `NO_SUBMISSION` 을 문서에 안 적어 계약 테스트가 잡았다.

**검증**: 백엔드 **1073** / 프론트 **140** / 빌드 / 접근성 0건 / 좁은화면 0건 /
E2E user-flow(오답 상세 단계 포함, Dice 0.9387)·error-paths 통과.
...
사용자 지적: **디자인 시스템(색·카드·라운드)만 입히고 프레임 자체는 그대로였다.**
맞는 지적이다. 특히 홈이 그랬다 — 시안 04 는 *인사말 + 일러스트* hero 인데
우리는 그 자리에 케이스 카드를 두고 있었다. 첫인상이 "목록"이 됐다.

**시안 04 대로 다시 만든 홈**
- hero: **인사말이 머리말**(34px 네이비) + 한 줄 안내 + teal 알약 버튼 + 오른쪽 일러스트
- 일러스트는 **직접 그렸다** (책·판독지·돋보기). 시안처럼 스톡 그림을 쓰지 않았고,
  **실제 환자 영상을 장식으로 쓰지도 않았다.**
- 시안에 없지만 남긴 것: `여기서 시작하세요 / 이어서 학습하기` 구분과 추천 이유.
  **이유 없는 추천은 신뢰받지 못한다** (테스트가 이걸 잡아 줬다)

**시안 05 대로 다시 만든 지표 카드**
둥근 칩이 왼쪽, 라벨 위에 큰 숫자, 진행바와 캡션이 카드 전체 폭.

**시안 08 대로 다시 만든 결과 요약**
`겹친 / 놓친 / 과하게 표시한` 을 **카드 세 장**으로. 아이콘 칩 + 큰 수치 + 진행바.
(시안의 종류/위치/ROI 정확도는 질환을 고르는 단계가 있어야 성립한다 — 우리에겐 없다)

**상단 메뉴**: 시안처럼 **가운데 정렬**, 현재 위치는 알약이 아니라 **밑줄**.

**검증**: 프론트 136 / 빌드 / 접근성 0건 / 좁은화면 0건 / E2E user-flow 통과.

---

### UI 개편 #3 — 남은 화면 + **키보드로 ROI 입력하기**

**키보드만으로 표시할 수 있게 했다 (새 기능).**
지금까지 이 캔버스는 "여기에 무엇이 있는지 읽을 수 있다"까지만 했고
실제로 칠할 방법은 마우스뿐이었다. 확대·이동·밝기 도구가 늘면서 그 간극이 더 벌어졌다.

모델은 **플로터처럼 펜을 내렸다 올리는 것**이다:
`←↑↓→` 이동(Shift 는 크게) / `Space` 펜 내리기·올리기 / `Enter` 박스 모서리 /
`[` `]` 굵기 / `Esc` 펜 올리기·박스 취소.
좌표는 포인터 입력과 **같은 원본 픽셀 좌표계**를 쓴다 — 다르면 같은 곳을 칠해도 채점이 달라진다.
브라우저에서 키보드만으로 3,026px 이 칠해지고 bbox 가 의도한 자리에 들어오는 것을 확인했다.
포커스를 잃으면 **펜을 반드시 올린다** (내려둔 채로 두면 다음 조작이 이어 그려진다).
화면 아래에 조작법을 적어 뒀다 — **적지 않으면 없는 것과 같다**(되돌리기에서 배운 것).

**남은 시안 화면 반영**
- **복습노트 → 시안 09 의 카드 그리드.** 한 줄 목록보다 영상이 먼저 보인다.
  버튼 이름을 `재도전` → `다시 풀기` 로 (판독 화면의 같은 동작과 이름을 맞췄다).
  `user-flow.mjs` 의 선택자도 함께 갱신했다.
- **계정 설정 → 시안 11 의 프로필 카드.** 시안의 학교·학과·직군 칸은 **넣지 않았다** —
  우리는 가입할 때 그런 정보를 받지 않는다. 없는 칸을 만들지 않는다.
- **회원가입 → 시안 02 의 단계 표시기.** 다만 **두 페이지로 쪼개지 않았다** —
  동의는 법적으로 반드시 받아야 해서 계정 정보와 떼어 놓지 않는다.
- 진행현황에 남아 있던 옛 문구(`부분 일치`/`불일치`)를 공용 라벨로 통일.

**시안 10(오답 상세)는 만들지 않았다.** 시안의 "내 답안 vs 정답"은 **질환명을 고르는 퀴즈**이고
(기관지염 vs 폐렴), 우리에겐 그 단계가 없다. "치료 원칙"도 우리가 쓸 수 없는 의학 내용이다.
우리의 대응물은 **재도전 화면**이고, 거기서는 실제로 다시 칠할 수 있다 — 더 낫다고 판단했다.

**검증**: 백엔드 1066 / 프론트 **135** / 접근성 0건 / 좁은화면 0건 /
E2E user-flow·roi-undo·error-paths 통과.

---

### UI 개편 #2 — 시안의 **없던 기능을 실제로 만들고**, 시안/기존 중 나은 쪽을 골랐다

사용자 지시: *"피그마 UI랑 다르다. 없는 기능은 만들어야지."* 그리고
*"둘 중 더 좋은 걸 채택해라. 무조건 피그마로만 할 필요 없다."*

**만든 기능 (시안 07 에 있었고 우리에겐 없던 것)**
- **뷰어 도구 레일**: 확대 / 축소 / 기본크기(1:1) / 이동(팬) / 밝기·대비 / 리셋
- **박스 ROI 도구** (펜 · 박스 · 지우개)
- **의학용어 사전 패널** + `GET /api/glossary` (신규 엔드포인트)

**확대·이동을 넣을 때 가장 걱정한 것은 "채점 좌표가 틀어지는가" 였다.**
`.stage` 에 CSS transform 으로만 걸었다. 캔버스 버퍼는 원본 해상도 그대로 두고,
좌표 변환은 `getBoundingClientRect()`(변환이 반영된 사각형)로 계산하므로
**확대·이동해도 화면 좌표 → 원본 픽셀 변환식이 그대로 성립한다** (회전이 없기 때문).
말로만 두지 않고 두 겹으로 고정했다:
  - 단위 테스트: 1.25배 확대 상태에서 화면 중앙을 찍으면 원본 중앙 좌표가 나온다
  - 브라우저 실측: 1.56배에서 캔버스 정중앙을 찍어 원본 (256,256) 픽셀이 칠해지는 것 확인
또한 **리셋은 보기만 되돌리고 칠한 ROI 는 지우지 않는다** (보기를 고치려다 작업을 잃으면 안 된다).
**이동(팬)은 입력이 잠긴 slice 에서도 쓸 수 있다** — 보는 것을 막을 이유가 없다.

**사전은 지어내지 않았다.** `content/diseases/vestibular_schwannoma.json` 에 이미
**출처가 달린 용어 6개**가 커밋돼 있었고 화면까지 오지 않았을 뿐이다. 그대로 서비스한다.
제출 전에 열려도 GT 가 새지 않는다 — 질환 일반론이고 이 케이스의 마스크·위치·크기는 없다
(`tests/test_glossary.py` 가 케이스별 필드가 섞이지 않는지 검사한다).
시안의 **발음 듣기(TTS) 아이콘은 넣지 않았다** — 한국어 TTS 로 영문 의학용어를 읽히면
틀린 발음을 가르치게 된다.
...
| 화면 | 가져온 것 |
|---|---|
| 전역 | teal + 네이비 팔레트, 둥근 모서리, 카드 언어 |
| 04 메인 | 흰 hero 카드(네이비 제목 + teal CTA), 바로가기 3카드 |
| 05 대시보드 | 아이콘 칩이 붙은 지표 카드, **최근 학습 활동 표**(썸네일 포함) |
| 06 학습 선택 | 흰 알약 필터 칩, 카드 하단의 눌리는 면 |
| 07 문제 풀이 | CASE SOLVING 헤더 · CASE 블록 · 과제 안내, 펜/박스/지우개 툴바, **툴바 안 제출**, 좌측 도구 레일, 좌하단 상태 표시, 우측 용어 사전 |
| 08 채점·해설 | 색이 채워진 판정 알약, 해설 섹션 아이콘 |

#### **기존 것이 낫다고 판단해 유지한 것** (시안을 따르지 않음)

1. **"정답/부분정답/오답" → 쓰지 않는다.** 우리는 의료인이 아니다 (개발원칙 3).
   `기준과 일치 / 일부 일치 / 기준과 다름`.
2. **`MedLens AI — SCORING AND EXPLANATION` 헤더 → 쓰지 않는다.**
   채점자는 전문가 GT 이고 AI 는 참고 정보다. 루프 #21 에서 막은 오해와 같은 것이다.
3. **케이스 목록: 3단계 선택(부위→영상→질환) → 기존 목록 유지.**
   우리 데이터로는 **각 단계에 선택지가 하나씩**이라 빈 껍데기가 된다.
   검색·상태 필터·시도 이력이 붙은 지금 목록이 6케이스에 더 맞는다.
4. **상단 네비게이션 유지.** 시안은 04/05/07/08 은 상단바, 06/09/11 은 좌측 사이드바로
   **스스로 일관되지 않다.** 우리 8개 화면은 상단바로 통일돼 있다.
5. **결과 요약: 종류/위치/ROI 정확도 → 겹친/놓친/과하게 표시한 유지.**
   시안의 "종류 정답" 은 질환을 고르는 단계가 있어야 성립한다(우리에겐 없다).
   그리고 **무엇을 고쳐야 하는지**는 우리 쪽이 말해 준다.
6. **해설 출처 3층 분리 유지** (`case_facts` / `disease_info` / `case_findings` + 출처 칩).
   시안은 문헌 일반론과 케이스 소견을 구분 없이 섞는다.
7. **slice 탐색 유지.** 시안에는 없다. 우리는 slice 18장이고 이게 훈련의 핵심이다.
8. **되돌리기/다시하기 유지.** 시안에는 없다.
9. **"W 4096 / L 2048" → 밝기·대비 %로 대체.** 우리 자산은 **이미 윈도잉을 마친 PNG** 라
   원본 HU 가 없다. 없는 값을 숫자로 적으면 거짓말이 된다.
10. **제출 뒤 우측 패널은 "다음 학습"으로 바뀐다.** 학습 루프를 닫는 장치를 버리지 않았다.

#### 만들지 않은 것 (데이터가 없어서)
- **임상 정보 vignette**("72세 남자가 내원 2일 전부터…") — 우리에게 없는 환자 병력이다.
  자리를 만들어 비워 두는 대신 **과제 안내 문구**로 대체했다(우리가 쓰는 조작 안내라 창작이 아니다).
- **케이스별 영상 소견**(`case_findings`) — 구조는 있고 전문가 검토 대기(BLOCKER-2).
- **모드(자율 학습/퀴즈) 칸** — 우리에게 모드 개념이 없다. 빈 칸을 만들지 않았다.

**이어서 더 반영한 것 (같은 사이클)**
- 상단 메뉴에 선 아이콘 (시안 04·05). **아이콘만 두지 않는다** — 항상 글자와 함께 쓰고,
  좁은 화면에서는 아이콘을 감추고 글자를 남긴다 (반대로 하면 그림만 보고 짐작해야 한다).
- **로그인 화면을 시안 01 의 좌우 분할로** — 왼쪽 네이비 브랜드 면 + 오른쪽 폼.
  **시안의 흉부 X-ray 배경은 쓰지 않았다.** 우리가 가진 영상은 실제 환자 의료영상이고
  장식으로 쓸 성격이 아니다. 격자·스캔선 모티프를 CSS 로 그렸다.
- **결과 하단에 다음 행동 버튼 줄** (시안 08). 오른쪽 패널은 화면 위쪽에 있어서
...
### UI 개편 #1 — MedLens 시안 반영 (디자인 토큰 + 홈 · 케이스목록 · 결과)

사용자가 Figma 시안(`MedLens 화면 디자인`, 13프레임)을 주고 **핵심 4화면
(홈·케이스목록·판독·결과)** 을 이 디자인으로 바꾸자고 했다.

**시안을 그대로 옮기면 안 되는 것 3가지를 먼저 걸렀다.**
1. 시안 전체가 **"정답 / 부분 정답 / 오답"** 을 쓴다 — 개발원칙 3 위반이다.
   사용자와 합의해 **절충**했다: 차갑고 판정처럼 읽히는 "불일치" 대신
   **무엇과 비교했는지가 주어로 드러나는 말**을 쓴다.
   `일치→기준과 일치` / `부분 일치→일부 일치` / `불일치→기준과 다름`.
   세 파일에 흩어져 있던 `GRADE_LABEL` 을 `labels.js` 로 모았다
   (`gradeLabel` = 문장용, `gradeBadge` = 좁은 자리용).
2. 시안 08 헤더가 **`MedLens AI — SCORING AND EXPLANATION`** 이다.
   "AI 가 채점한다"고 말하는 화면이다 — 루프 #21 에서 로그인 태그라인을 고친 것과
   **정확히 같은 오해다.** 쓰지 않았다.
3. 시안 콘텐츠가 전부 **흉부 X-ray**(폐 결절·기흉·흉막삼출)다. 우리 실데이터는
   뇌 MRI 6케이스뿐이고 흉부 합성 케이스는 서비스에서 제거했다.
   시안 06 의 3단계 선택(부위→영상→질환)은 **각 단계 선택지가 하나씩뿐**이라
   빈 껍데기가 된다 — 사용자 결정으로 **기존 케이스 목록을 새 디자인으로** 입혔다.

**시안의 teal 은 흰 글씨 대비가 2.88:1 이라 그대로 쓸 수 없었다** (AA 4.5:1 미달).
색상(hue)은 시안 그대로 두고 **글씨가 얹히는 면만** 어둡게 나눴다:
`--brand-400 #33a7af`(시안 원본, 장식·아이콘) / `--brand-500 #1f7b82`(흰 글씨 버튼, 4.98:1) /
`--brand-600·700`(hover·teal 텍스트, 6.4:1·8.6:1). 색은 시안 화면을 **픽셀에서 직접 추출**했다.
결과 판정 알약도 마찬가지다 — 시안의 coral(#f67566, 2.74:1) 대신 이미 대비를 맞춰 둔
`--*-ink` 를 채움색으로 썼다(5.7~7.0:1).

**한 것**
- `style.css` 토큰 교체: teal 브랜드 + 네이비(`--navy-*`) + 차가운 회색 + 둥근 모서리(8/12/16)
- 홈: 시안 04 구성으로 — **흰 hero 카드**(네이비 제목 + teal 버튼 + 실제 케이스 영상) +
  바로가기 3카드 + 아이콘 칩이 붙은 지표 카드
- 케이스 목록: 필터를 시안의 **흰 알약 칩**으로, 카드 하단을 눌리는 teal 면으로
- 결과: 판정을 시안의 **색이 채워진 알약**으로

**판독 화면은 토큰만 반영했다.** 시안 07 의 나머지 요소는 전부
**우리에게 없는 기능·콘텐츠**다 — 확대/축소/팬/밝기 도구, 의학용어 사전 패널,
임상 정보 문구(vignette). 지어내지 않고 남겨 뒀다 (개발원칙 4).

**검증**: 프론트 108 passed / 접근성 0건 / 좁은화면 0건 /
E2E user-flow·roi-undo·slice-navigation 통과 (새 문구로 통과하는 것까지 확인).
...
- ~~키보드로 ROI 입력할 수단~~ → **완료** (UI 개편 #3).
  화살표 이동 / Space 펜 내리기·올리기 / Enter 박스 모서리 / `[` `]` 굵기 / Esc.
  좌표계는 포인터와 동일하다.
- `Submission.explanation` 스냅샷을 어디에 보여줄지
  → **지금은 만들지 않는다.** `case_findings` 가 전부 null 이라 "당시 해설"과
  "지금 해설"이 같다 — 화면에 붙여도 빈 블록만 나온다.
  전문가 소견이 들어오고 **개정이 한 번이라도 일어난 뒤**에 붙이는 것이 맞다.
  (컬럼은 의도적으로 남겨 둔다 — `models.py` 주석 참고)
- CSP 정책 확정 (배포 형태가 정해진 뒤 `MEDISCAN_CSP` 로)
- 본격 부하 테스트
- 시안 10(오답 상세)은 **의도적으로 만들지 않았다** — 질환명을 고르는 퀴즈
  전제라 우리 구조와 맞지 않는다 (UI 개편 #3 참고)
...
>
> - [ ] **커밋 4개가 로컬에만 있다** (`c714c3a` `8386435` `c676fc9` `fdb0723`).
>       push 는 사용자 승인이 필요하다 — 아직 받지 않았다.
> - [ ] **`review_area_ratio = 2.0` 은 Claude 가 정한 값이다.** Dice 임계값과 같은 지위로
>       "미검증"이라 표시하고 `MEDISCAN_REVIEW_AREA_RATIO` 로 덮게 해 뒀지만,
>       **실제 값은 검토가 필요하다** (E3 와 함께 본다).
> - [ ] **모바일 실기기 확인.** 도구 레일을 영상 아래로 내렸고 좁은화면 점검은 통과하지만,
>       **손가락으로 실제로 칠하는 감촉은 검증하지 못했다** (자동화가 JS 이벤트로 돌았다).
> - [ ] **케이스 구별 근거가 없다** — 6케이스가 전부 `뇌 MRI · 전정신경초종`이고 `difficulty` 는
>       전부 null 이다. 병변 크기 등을 대신 쓰면 정답 크기가 새므로 **E2(전문가 난이도 판정) 대기**다.
