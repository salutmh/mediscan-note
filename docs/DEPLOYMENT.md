...
| 11 | SNS 로그인 | 각 사 앱 ID 를 넣으면 실검증이 켜진다. **미설정 제공자는 503 으로 거부**된다 |
| 12 | 보안 헤더·자산 서명 확인 | `curl /health` 의 `security_headers` / `asset_urls`. 케이스 영상은 서명 URL 로만 받을 수 있다 |
| 13 | `MEDISCAN_RATE_LIMIT` 확인 | `0` 은 거부된다. 앞단에서 제한한다면 `external` 로 명시 |
