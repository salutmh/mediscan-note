...
.viewer-col {
  /* 영상 폭.
     예전에는 `min(100%, calc(100vh - 300px))` 였다. 세로로 잘리지 않게 하려던
     것인데, **창이 낮으면 가로까지 같이 줄어들어** 영상이 280px 로 쪼그라들고
     왼쪽에 빈 여백만 크게 남았다 (시안은 영상이 화면의 절반 이상을 쓴다).

     그래서 **영상이 칸을 그대로 채운다** (시안 07 도 그렇다).
     낮은 창에서는 영상 아래가 조금 잘려 스크롤이 생기지만, 그건 스크롤로 해결되는
     문제이고 영상이 손톱만 해지는 것보다 판독에 낫다.
     세로가 넉넉한 화면에서는 어차피 칸 폭이 먼저 한계가 된다. */
  --roi-max-width: 100%;
}
